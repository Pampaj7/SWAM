...
def retrive_data(save=False):
    ...
    csv_files = glob.glob(
        os.path.join(base_directory, "**/emissions_detailed.csv"), recursive=True
    )
    df_list = []
...
def plot(
    ...
    if plotType == "histogram":
        ...
        for i, col in enumerate(hist_cols):
            ...
            sns.boxplot(
                x=df[col],
                color="skyblue" if i == 0 else "orange" if i == 1 else "green",
            )
            plt.title(f"Distribuzione di {col}")
        plt.tight_layout()
        if save_path:
...
def mean_unique_quadruplets(df: pd.DataFrame, columns: list[str]):
    ...
    result = (
        df.groupby(["algorithm", "dataset", "language", "phase"])[columns]
        .mean()
        .reset_index()
    )
...
def median_unique_quadruplets(df: pd.DataFrame, columns: list[str]):
    ...
    result = (
        df.groupby(["algorithm", "dataset", "language", "phase"])[list(columns)]
        .median()
        .reset_index()
    )
...
def mean_group_by(df: pd.DataFrame, group_by: str, columns: list[str]):
    ...
    if group_by not in ["algorithm", "dataset", "language"]:
        raise ValueError(
            "group_by must be one of 'algorithm', 'dataset', or 'language'"
        )
...
def sum_metrics_per_algorithm(df):
    ...
    result = pd.merge(
        sum_train, sum_test, left_on="train_algorithm", right_on="test_algorithm"
    )
