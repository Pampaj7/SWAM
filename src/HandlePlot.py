...
def plot_execution_time_by_language_and_algorithm(
    ...
    grouped_data = (
        filtered_df.groupby([algorithm_col, language_col])[time_col]
        .sum()
    ...
    ax = grouped_data.plot(
        kind="bar", figsize=(14, 8), colormap="tab20", width=0.8, fontsize=14
    )
    plt.legend(
        title="Languages", fontsize=18, title_fontsize=20
...
def plot_duration_vs_emissions(
    ...
    df_filtered = df[
        ~((df[co2_col] < (Q1 - 1.5 * IQR)) | (df[co2_col] > (Q3 + 1.5 * IQR)))
    ]

    plt.figure(figsize=(12, 8))
    sns.scatterplot(
        x=duration_col, y=co2_col, data=df_filtered, hue="language", palette="tab20"
    )
    plt.grid(True)
...
def plot_performance_heatmap(df, language_col, algorithm_col, col):
    for phase in ["train", "test"]:
        ...
        pivot_table = phase_df.pivot_table(
            values=col, index=algorithm_col, columns=language_col, aggfunc="mean"
        )
        ...
        ax = sns.heatmap(
            ...
            fmt=".1e",
            cmap="viridis",
            cbar_kws={"label": "Energy consumed (Joules)"},
            annot_kws={"size": 20},
        )
        ...
        colorbar.ax.tick_params(
            labelsize=20
        )  # Change font size of the tick labels on color bar
        colorbar.set_label(
            "Energy consumed (Joules)", fontsize=20
...
def plot_correlation_by_phase(
    ...
    fig, axes = plt.subplots(
        nrows=len(phases), ncols=1, figsize=(10, 5 * len(phases)), sharex=True
    )
...
def plot_time_series_all_algorithms(
    ...
    if df_filtered.empty:
        print(
            f"No data available for language: {language}, dataset: {dataset} in phase: {phase}"
        )
        return
...
def plot_execution_time_by_language(
    ...
    if not train_df.empty:
        grouped_train = (
            train_df.groupby([language_col, "dataset"])[time_col].sum().unstack()
        )
    ...
    if not test_df.empty:
        grouped_test = (
            test_df.groupby([language_col, "dataset"])[time_col].sum().unstack()
        )
...
def plot_time_series_subplot(df, combinations, date_col, value_col, phase_col="phase"):
    ...
    for idx, (language, dataset, algorithm) in enumerate(combinations):
        ...
        if df_filtered.empty:
            print(
                f"No valid data available for language: {language}, dataset: {dataset}, algorithm: {algorithm}"
            )
            continue
