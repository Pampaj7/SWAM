...
file_path = [
    "python/train_emissions_detailed.csv",
    "python/test_emissions_detailed.csv",
...
def run_algorithms(X, y, dataset_name):
    results = {
        name: {"accuracy": 0, "fit_emissions": 0, "predict_emissions": 0}
    ...
    for model, name in algorithms.items():
        ...
        for epoch in range(epochs):
            ...
            fit_tracker = EmissionsTracker(
                output_dir="python", output_file=f"train_{file_name}"
            )
            fit_tracker.start()
            ...
            predict_tracker = EmissionsTracker(
                output_dir="python", output_file=f"test_{file_name}"
            )
