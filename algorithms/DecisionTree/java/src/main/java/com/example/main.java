...
public class main {
  ...
  public static void main(String[] args) {
    try {
      ...
      data.setClassIndex(1); // Assuming the class label is the second attribute (index 1)

      // Stratify and split the data: 80% train, 20% test with random_state=42
      Instances[] split = stratifiedSplit(data, 0.8, 42);
      Instances train = split[0];
      Instances test = split[1];

      // Create and train the decision tree (J48) classifier with random_state=42
      ...
      // Output the accuracy
      System.out.println("Accuracy: " + (evaluation.pctCorrect() / 100) + "\n");
   ...
   * @param trainRatio The ratio of the dataset to be used for training (e.g., 0.8
   *                   for 80% train).
   * @param randomSeed The random seed to ensure reproducibility.
   * @return An array containing the training set at index 0 and the test set at
   *         index 1.
  ...
  public static Instances[] stratifiedSplit(Instances data, double trainRatio, int randomSeed) throws Exception {
    ...
    filter.setNumFolds((int) (1 / (1 - trainRatio)));
    filter.setFold(1); // 1 for the train set
