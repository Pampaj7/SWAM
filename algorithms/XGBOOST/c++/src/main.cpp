...
void split_data(const std::vector<std::vector<float>> &X, const std::vector<int> &y,
                std::vector<std::vector<float>> &X_train, std::vector<int> &y_train,
    ...
    // Shuffle indices
    auto rng = std::default_random_engine{static_cast<unsigned>(std::chrono::system_clock::now().time_since_epoch().count())};
...
void train_and_evaluate_xgboost(const std::vector<std::vector<float>> &X,
    ...
    XGDMatrixCreateFromMat(flattened_X_train.data(), trainRows, trainCols, -999.0f, &dtrain);
    ...
    const char *param_keys[] = {"booster", "objective", "eval_metric", "max_depth", "eta"};
    const char *param_values[] = {"gbtree", "binary:logistic", "error", "6", "0.3"};
    ...
    DMatrixHandle dtest;
    XGDMatrixCreateFromMat(flattened_X_test.data(), testRows, trainCols, -999.0f, &dtest);
